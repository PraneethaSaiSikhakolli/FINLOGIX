...
import {
  ...
  FaChartPie, FaUserCog, FaChartLine, FaPlus
} from 'react-icons/fa';
...
const AdminPanel = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 text-gray-900 flex relative overflow-hidden">
      {/* Floating blobs for premium aesthetic */}
      ...
      {/* Sidebar */}
      <aside className="w-60 min-h-full bg-white border-r border-gray-200 p-6 shadow-md z-10">
        ...
        <nav className="flex flex-col gap-4">
          {tabs.map((tab, i) => (
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className={`flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all duration-300 text-sm font-semibold tracking-wide
                ${activeTab === i
                  ? 'bg-black text-white shadow-lg'
                  : 'text-gray-800 hover:bg-neutral-100 hover:text-black'}`}
            >
              {tab.icon}
              <span>{tab.name}</span>
            </motion.button>
      ...
      {/* Main Content */}
      <main className="flex-1 flex flex-col items-center px-6 md:px-12 py-8 z-10">
        <motion.h1
          className="text-4xl md:text-5xl font-bold text-center mb-2 text-black tracking-tight"
        ...
        <p className="text-gray-500 mb-6 text-sm italic tracking-wide">{breadcrumb}</p>

        <section className="w-full max-w-6xl bg-white border border-gray-200 rounded-3xl shadow-lg p-6 md:p-8 mb-24 relative">
